-- Update places_with_merged_data view to include review_analysis

-- Drop existing view first (can't change column order with CREATE OR REPLACE)
DROP VIEW IF EXISTS places_with_merged_data;
...
SELECT
  ...
  -- Review analysis (NEW - from places table)
  p.review_analysis,
...
COMMENT ON VIEW places_with_merged_data IS 'Merged view of all place data with override precedence and review analysis';
